...
namespace FHIRtoCDM
{
    public class CdmPersonBuilder : IPersonBuilder
    {
        #region Variables

        private readonly Dictionary<long, VisitOccurrence> _rawVisits = new Dictionary<long, VisitOccurrence>();
        ...
        public List<EraEntity> ObservationPeriodsRaw = new List<EraEntity>();
        ...
        {
            if (!_rawVisits.ContainsKey(rawVisit.Id))
                _rawVisits.Add(rawVisit.Id, finalVisit);
            else
                _rawVisits[rawVisit.Id] = finalVisit;
        }


        public IEnumerable<VisitOccurrence> BuildVisitOccurrences(VisitOccurrence[] rawVisitOccurrences,
            ObservationPeriod[] observationPeriods)
        {
            ...
            foreach (
                ...
                rawVisitOccurrences.Where(
                    visitOccurrence => visitOccurrence.ConceptId != 9201))
            {
        ...
        {
            if (ent.VisitOccurrenceId.HasValue && _rawVisits.ContainsKey(ent.VisitOccurrenceId.Value))
            {
                return _rawVisits[ent.VisitOccurrenceId.Value];
            }

            return null;
        }

        public IEnumerable<VisitDetail> BuildVisitDetails(VisitDetail[] visitDetails,
            ...
            foreach (var visitOccurrence in visitOccurrences)
            {
                var visitDetail =
                    new VisitDetail(visitOccurrence)
                    {
                        ...
                        VisitOccurrenceId = visitOccurrence.Id
                    };
        ...
        {
            foreach (
                var eraEntity in EraHelper.GetEras(
        ...
        public virtual Attrition Build(ChunkData data, KeyMasterOffsetManager o)
        {
            ...
            foreach (var item in BuildDrugExposures(DrugExposuresRaw.ToArray(), visitOccurrences, observationPeriods))
            {
                item.Id = Offset.GetKeyOffset(item.PersonId).DrugExposureId;
                drugExposures.Add(item);
            }

            var conditionOccurrences = new List<ConditionOccurrence>();
            foreach (var item in BuildConditionOccurrences(ConditionOccurrencesRaw.ToArray(), visitOccurrences, observationPeriods))
            {
                item.Id = Offset.GetKeyOffset(item.PersonId).ConditionOccurrenceId;
                conditionOccurrences.Add(item);
            }

            var procedureOccurrences = new List<ProcedureOccurrence>();
            foreach (var item in BuildProcedureOccurrences(ProcedureOccurrencesRaw.ToArray(), visitOccurrences, observationPeriods))
            {
                item.Id = Offset.GetKeyOffset(item.PersonId).ProcedureOccurrenceId;
                procedureOccurrences.Add(item);
            }

            var observations = new List<Observation>();
            foreach (var item in BuildObservations(ObservationsRaw.ToArray(), visitOccurrences, observationPeriods))
            {
                item.Id = Offset.GetKeyOffset(item.PersonId).ObservationId;
                observations.Add(item);
            }

            var measurements = new List<Measurement>();
            foreach (var item in BuildMeasurement(MeasurementsRaw.ToArray(), visitOccurrences, observationPeriods))
            {
                item.Id = Offset.GetKeyOffset(item.PersonId).MeasurementId;
            ...
            // set corresponding PlanPeriodIds to drug exposure entities and procedure occurrence entities
            SetPayerPlanPeriodId(payerPlanPeriods, drugExposures.ToArray(), procedureOccurrences.ToArray(),
            ...
            mins.Add(GetMinDate(visitDetails));
            ...
            maxs.Add(GetMaxDate(visitDetails));
            ...
            SetVisitOccurrenceId(drugExposures);
            SetVisitOccurrenceId(conditionOccurrences);
            SetVisitOccurrenceId(procedureOccurrences);
            SetVisitOccurrenceId(measurements);
            SetVisitOccurrenceId(observations);
            SetVisitOccurrenceId(deviceExposure);

            // push built entities to ChunkBuilder for further save to CDM database
            AddToChunk(person, death, observationPeriodsFinal.ToArray(), payerPlanPeriods, drugExposures.ToArray(),
                conditionOccurrences.ToArray(), procedureOccurrences.ToArray(), observations.ToArray(), measurements.ToArray(),
        ...
        private void SetVisitOccurrenceId(IEnumerable<IEntity> entities)
        {
            Parallel.ForEach(entities, e =>
            {
                var vo = GetVisitOccurrence(e);
        ...
        public bool Excluded(Person person, IEnumerable<ObservationPeriod> periods)
        {
            return false;
        }
